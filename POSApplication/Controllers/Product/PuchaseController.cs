...
namespace POSApplication.Controllers
{
    public class PuchaseController : Controller
    {
        ...
        public ActionResult Create(Purchase p, string InvoicePrint)
        {
            ...
            decimal pa = Convert.ToDecimal(p.PayableAmount);
            decimal? ta = Convert.ToDecimal(p.TotalAmount);
            int? taa = Convert.ToInt32(p.TaxPer);

            var a = db.usp_Add_Purchase(BranchID, IsTransferFromBranch, TransferFromBranchID, PoNumber, PoDate, PoReference, SupplierID, p.PaymentMode, ta, taa, p.TotalAmount, pa, CreatedBy);
            if (a.ToString() != "0")
            {
        ...
        public FileResult Print(Purchase p)
        {
            try
            {
                ...
                rd.Load(Path.Combine(Server.MapPath("Reports"), "PurchaseReport.rpt"));
                List<puchaseaddtocartclass> li2 = Session["cart"] as List<puchaseaddtocartclass>;
                foreach (var item in li2)
                {
                ...
                //Report.Report.PurchaseInvoiceReport rep = new Report.Report.PurchaseInvoiceReport();
                //rep.Da
                ...
                return File(st, "application/pdf", "PurchaseInvoice" + DateTime.Now.ToString("d"));
            }
            catch (Exception msg)
            {
        ...
        {
            int id = Int32.Parse(Session["BranchID"].ToString());
            int PoNum = db.Purchases.Where(x => x.BranchID == id).ToList().Select(x => x.PurchaseID).Max();
